...
import useContextInfo from "../../Layout/UserLayout/Hooks/useContextInfo";
const About = () => {
  ...
  return (
    ...
    >
      {navigationData.map((navigation, idx) => (
        <li
          key={idx}
          className="group flex w-full list-none px-2 gap-2 items-center rounded-md"
        >
          <Link
            to={navigation.link}
            className={`flex items-center gap-2 group-hover:${textColor}`}
          >
            {React.createElement(iconName[navigation?.icon_name], {
              className: `group-hover:${textColor} text-2xl ${navigation?.bg}`,
            })}
            {navigation?.name}
          </Link>
        </li>
